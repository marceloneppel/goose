...
fn get_log_directory_with_date(test_date: Option<String>) -> Result<PathBuf> {
    // We can't use goose::logging::get_log_directory_with_date because it's only available
    // in test mode for the goose crate itself. Instead, we'll replicate the logic here.
